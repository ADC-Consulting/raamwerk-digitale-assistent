...
Dwing toegang centraal af via een gateway met fail-closed allowlist: een MCP-gateway of vergelijkbare laag bepaalt — buiten het manipuleerbare model om — welke agent onder welke voorwaarden bij welke systemen en endpoints kan, en routeert alle outbound API-calls. Werk daarbij met allowlists in plaats van blocklists: een blocklist vergeet altijd iets, terwijl een endpoint die niet expliciet op de allowlist staat buiten bereik blijft.
